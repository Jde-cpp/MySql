...
namespace Jde::DB::MySql
{
	struct MySqlDataSource final: public IDataSource
	{
		...
		[[noreturn]] α SetAsynchronous()noexcept(false)->void override{ THROW2("Not Implemented"); }
		...
		α Execute( sv sql, const vector<DataValue>* pParameters, std::function<void(const IRow&)>* f, bool isStoredProc=false, bool log=true )noexcept(false)->uint override;
	...
	private:
		α Select( sv sql, std::function<void(const IRow&)> f, const vector<DataValue>* pValues, bool log )noexcept(false)->uint override;
	};
