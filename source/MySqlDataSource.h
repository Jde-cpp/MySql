...
namespace Jde::DB::MySql
{
	struct MySqlDataSource final : IDataSource
	{
		...
		α ExecuteProcCo( string sql, vector<object> p, SL sl )noexcept->up<IAwait> override;
		α SelectCo( ISelect* pAwait, string sql, vector<object>&& params, SL sl )noexcept->up<IAwait> override;
